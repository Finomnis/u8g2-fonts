...
impl FontRenderer {
    ...
    */
    ...
    /// # Return
    ///
    /// The [`DrawBuilder`] to perform the actual drawing.
    ///
    pub fn render_glyph(&self, ch: char) -> DrawBuilder<GlyphContent, (), ()> {
        DrawBuilder::new(&self.font, GlyphContent(ch))
    }
    ...
    /// The [`DrawBuilder`] to perform the actual drawing.
    ///
    pub fn render_text<'a>(&self, text: &'a str) -> DrawBuilder<TextContent<'a>, (), ()> {
        DrawBuilder::new(&self.font, TextContent(text))
    }
    ...
    /// The [`DrawBuilder`] to perform the actual drawing.
    ///
    pub fn render_args<'a>(&self, args: Arguments<'a>) -> DrawBuilder<ArgsContent<'a>, (), ()> {
    ...
    pub fn get_glyph_dimensions(
        ...
        let bounding_box = (size.width > 0 && size.height > 0)
            .then(|| glyph.create_renderer().get_glyph_bounding_box(position));
